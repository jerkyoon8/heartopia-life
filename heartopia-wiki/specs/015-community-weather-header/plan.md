...
날짜와 슬롯으로 식별되는 사용자 현재 표와 실제 값 변경 이력을 별도 테이블에 저장한다. 공개 조회 서비스는 아시아 서버 현재 시각을 기준으로 상세 5칸과 기본 7일 키를 만들고, 가중 집계 결과를 상태(`CONFIRMED`, `TIED`, `EMPTY`)로 변환한다. 상세 결과가 비어 있을 때만 같은 날짜의 기본 결과를 대체값으로 사용한다.
...
- 서비스 단위 테스트
  ...
  - 일반 6표가 관리자 5점을 역전
  - 최고 점수 동점
  - 상세 EMPTY의 날짜 기본 날씨 대체와 TIED 미대체
